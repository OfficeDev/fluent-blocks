...
async function bundle() {
  ...
  await access(out).then(function unlinkOut() {
    return unlink(out)
  })
  ...
  await Promise.all(
    Object.keys(manifest.include).map(async function addIncludedAsset(id) {
      const sizes = manifest.include[id].sizes
        ? globSection(manifest.include[id].sizes)
        : fallbackSizes
      const styles = manifest.include[id].styles
        ? globSection(manifest.include[id].styles)
        : fallbackStyles

      const svgs = await glob(
        resolve(cwd, argv.svgs, `${id}_${sizes}_${styles}.svg`)
      )

      return Promise.all(
        svgs.map(async function addResolvedSvgs(svg) {
